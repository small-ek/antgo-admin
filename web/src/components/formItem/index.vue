...
<template>
  ...
  <a-form-item v-if="props.row.type==='input'" :field="props.row.key" :label="props.row.label+'：'" feedback>
    <a-input v-model="props.form[props.row.key]" autocomplete="off" class="input" placeholder="请输入" allow-clear/>
  </a-form-item>
  <!--数字框-->
  <a-form-item v-if="props.row.type==='number'" :field="props.row.key" :label="props.row.label+'：'" feedback>
    <a-input-number v-model="props.form[props.row.key]" class="input" placeholder="请输入" allow-clear/>
  </a-form-item>
  <!--下拉框-->
  <a-form-item v-if="props.row.type==='select'" :field="props.row.key" :label="props.row.label+'：'" feedback>
    <a-select v-model="props.form[props.row.key]" placeholder="请选择" allow-clear class="input">
